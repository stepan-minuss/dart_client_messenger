...
Основные зависимости указаны в `pubspec.yaml`:
- `provider` - управление состоянием
- `socket_io_client` - Socket.IO клиент
...
- Токены хранятся в Flutter Secure Storage
- Ключи шифрования генерируются и хранятся локально
- Сообщения шифруются перед отправкой
- Поддержка E2EE для приватных сообщений

## Локализация

Приложение поддерживает русский язык. Локализация находится в `lib/l10n/`.

## Лицензия

Приложение разработано для использования в мессенджере.
